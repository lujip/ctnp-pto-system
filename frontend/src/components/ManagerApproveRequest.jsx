...
function ManagerApproveRequest() {
  ...
  const fetchManagerRequests = async ({ silent = false } = {}) => {
    if (!managerId) {
  ...
  const submitStatusUpdate = async (requestId, action, comments = '') => {
    try {
      ...
      if (response.ok) {
        await fetchManagerRequests({ silent: true });
        return true;
      }
          ...
          <div className="reject-modal" onClick={(e) => e.stopPropagation()}>
            ...
            <form onSubmit={handleRejectSubmit} className="reject-modal-form">
              ...
              <div className="form-group">
                <label htmlFor="manager_reject_comments">Reason for rejection (optional)</label>
                <textarea
